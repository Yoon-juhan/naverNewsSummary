...
class UrlCrawling:
    ...
    def getSixUrl(self):    # 정치, 경제, 사회, 생활/문화, 세계, IT/과학
        ...
        for category in tqdm(range(6), desc="6개 카테고리 링크"):     # 6
            a_list = []
    ...
    def getEntertainmentUrl(self):   # 연예
        ...
        for page in tqdm(range(1, 5), desc="연예 링크"):  # 1, 5
    ...
    def getSportsUrl(self):    # 스포츠  (페이지마다 개수가 달라서 6페이지를 이동)
        ...
        for page in tqdm(range(1, 7), desc="스포츠 링크"):  # 1, 7
...
class ContentCrawling:
    ...
    def getSixContent(self, url_list):  # 정치, 경제, 사회, 생활/문화, 세계, IT/과학
        ...
        for url in tqdm(url_list, desc="6개 카테고리 본문"):
            browser.get(url)
        ...
        for i in range(len(title_list)):
            ...
            self.content.append(cleanContent(content_list[i].text))
    ...
    def getEntertainmentContent(self, url_list):    # 연예
        ...
        for url in tqdm(url_list, desc="연예 본문"):
            browser.get(url)
    ...
    def getSportsContent(self, url_list):   # 스포츠
        ...
        for url in tqdm(url_list, desc="스포츠 본문"):
